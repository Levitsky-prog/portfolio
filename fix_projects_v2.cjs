...
// Define our own constant and use it
const projectsStr = `const myProjects = ${JSON.stringify(projects)};`;

// Find the place to inject it (before P)
if (content.includes('const m=')) {
    // Replace the old m definition and inject our myProjects
    content = content.replace(/const m=\[\[.*?\]\]/, `${projectsStr} const m = [myProjects]`);
}

// Update loadAllProjects to use myProjects (flat array)
content = content.replace(/loadAllProjects:async t=>{.*?return n\.value\.set\(i,f\),f}/, 
    `loadAllProjects:async t=>{const c=t||"en",i=o("projects",c,"all");if(n.value.has(i))return n.value.get(i);const f=myProjects;return n.value.set(i,f),f}`);

// Update loadProjectBySlug to use myProjects
content = content.replace(/loadProjectBySlug:async\(t,c\)=>{.*?return n\.value\.set\(l,h\?\[h\]:\[\]\),h}/,
    `loadProjectBySlug:async(t,c)=>{const i=c||"ru",l=o("projects",i,\`slug-\${t}\`);if(n.value.has(l)){const u=n.value.get(l);return(u&&u.length>0?u[0]:null)||null}const h=myProjects.find(u=>u.meta?.slug===t)||null;return n.value.set(l,h?[h]:[]),h}`);

fs.writeFileSync(targetFile, content);
console.log('Fixed 5AFuoeiE_fix.js with flat array and unique variable name.');
